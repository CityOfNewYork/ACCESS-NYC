...
class Settings
{
    ...
    public function __construct($parent)
    {
        ...
        add_action('admin_notices', [$this, 'got_api_token']);

        add_action('admin_enqueue_scripts', [$this, 'admin_enqueue']);
    
        add_action('add_option_' . $this->parent->OPT_SCANNING_INTERVAL, [$this, 'schedule_event'], 10, 2);
        add_action('update_option_' . $this->parent->OPT_SCANNING_INTERVAL, [$this, 'schedule_event'], 10, 3);
        add_action('update_option_' . $this->parent->OPT_SCANNING_TIME, [$this, 'schedule_event'], 10, 3);

        add_action('update_option_' . $this->parent->OPT_IGNORE_ITEMS, [$this, 'update_ignored_items'], 10, 3);
    }
    ...
    {
        register_setting($this->page, $this->parent->OPT_API_TOKEN, [$this, 'sanitize_api_token']);
    ...
    public function sanitize_api_token($value)
    {
        ...
        if ( ! empty($errors) ) {
            ...
            update_option($this->parent->OPT_ERRORS, []); # Clear errors
        } else {
            if ( $this->parent->is_interval_scanning_disabled() ) {
                wp_clear_scheduled_hook($this->parent->WPSCAN_SCHEDULE);
            }
