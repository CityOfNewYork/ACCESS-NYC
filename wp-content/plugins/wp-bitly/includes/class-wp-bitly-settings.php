...
class Wp_Bitly_Settings {
	...
	public function register_settings() {
		...
		/**
		 * Output the authorize settings field.
		 *
		 * @param array $args Arguments passed to the settings field callback.
		 */
		function _f_settings_field_authorize( array $args ) {
			...
			if ( $wp_bitly_auth->is_authorized() ) {
				// Output a green connected button.
				$output  = sprintf( '<span id="connected_feedback"><span class="icon"><img src="%s"></span><span class="authorized">%s</span></span>', esc_url( $image ), __( 'Connected', 'wp-bitly' ) );
				...
				$output .= sprintf( '<span id="connected_feedback" class="hidden"><span class="icon"><img src="%s"></span><span class="authorized">%s</span></span>', esc_url( $image ), __( 'Connected', 'wp-bitly' ) );
		...
		/**
		 * Output the OAuth token settings field.
		 *
		 * @param array $args Arguments passed to the settings field callback.
		 */
		function _f_settings_field_oauth( array $args ) {
			...
			$auth_css = $wp_bitly_auth->is_authorized() ? ' class="authorized" ' : ' class="not_authorized" ';
		...
		/**
		 * Output the post types settings field.
		 *
		 * @param array $args Arguments passed to the settings field callback.
		 */
		function _f_settings_field_post_types( array $args ) {
			...
			foreach ( $post_types as $label ) {
				$output .= '<label for "' . $label . '><input type="checkbox" name="wpbitly-options[post_types][]" value="' . $label . '" ' . checked(
					in_array( $label, $current_post_types, true ),
					true,
					false
				) . '>' . $label . '</label><br>';
			}
		...
		/**
		 * Output the default organization settings field.
		 *
		 * @param array $args Arguments passed to the settings field callback.
		 */
		function _f_settings_field_default_org( array $args ) {
			...
			$output .= $wp_bitly_settings->get_org_options();
			$output .= '</select>';
			$output .= '</fieldset>';
		...
		/**
		 * Output the default group settings field.
		 *
		 * @param array $args Arguments passed to the settings field callback.
		 */
		...
		/**
		 * Output the default domain settings field.
		 *
		 * @param array $args Arguments passed to the settings field callback.
		 */
		...
		/**
		 * Output the debug settings field.
		 *
		 * @param array $args Arguments passed to the settings field callback.
		 */
		function _f_settings_field_debug( array $args ) {
			...
			$output .= '<label title="debug"><input type="checkbox" id="debug" name="wpbitly-options[debug]" value="1" ' . checked( $wp_bitly_options->get_option( 'debug' ), 1, 0 ) . '><span> ' . __(
				"Let's debug!",
				'wpbitly'
			) . '</span></label><br>';
			$output .= '<p class="description">';
			// translators: %s is a URL to the support forums.
	...
	public function get_org_options() {
		...
		foreach ( (array) $organization_response['organizations'] as $org ) {
			...
			$output .= "<option value = '$guid' $selected >" . esc_html( $name ) . '</option>';
		}
		$allowed_html = array(
			'option' => array(
				'value'    => array(),
	...
	public function get_group_options( $current_default_org = '' ) {
		...
		if ( wp_doing_ajax() ) {
			$org = sanitize_text_field( wp_unslash( isset( $_POST['curr_org'] ) ? $_POST['curr_org'] : '' ) );
		} else {
	...
	public function get_domain_options( $current_default_group = '' ) {
		...
		if ( wp_doing_ajax() ) {
			$group_id = sanitize_text_field( wp_unslash( isset( $_POST['curr_group'] ) ? $_POST['curr_group'] : '' ) );
		} else {
