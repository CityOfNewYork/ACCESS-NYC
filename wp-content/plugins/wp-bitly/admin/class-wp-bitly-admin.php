...
class Wp_Bitly_Admin {
	...
	public function enqueue_scripts( $hook ) {
		...
		if ( 'options-writing.php' === $hook ) {
			...
			wp_localize_script(
				$this->plugin_name . 'wp-bitly-admin-js',
				'wpBitlyData',
				array(
					'nonce' => wp_create_nonce( 'wpbitly_settings' ),
				)
			);
		}
	 ...
	 * @since   2.0
	 *
	 * @param   array $links An array returned by WordPress with our plugin action links.
	 *
	 * @return  array The slightly modified array.
	 */
	...
	public function display_settings_notice() {
		...
		if ( 'plugins' !== $screen->base || $this->wp_bitly_auth->is_authorized() ) {
			return;
		}
	...
	public function check_for_authorization() {
		...
		if ( ! $this->wp_bitly_auth->is_authorized() && isset( $_GET['access_token'] ) && isset( $_GET['login'] ) ) {

			$token = sanitize_text_field( wp_unslash( $_GET['access_token'] ) );
			$login = sanitize_text_field( wp_unslash( $_GET['login'] ) );

			$this->wp_bitly_logger->wpbitly_debug_log(
				array(
					'Referer'      => isset( $_SERVER['HTTP_REFERER'] ) ? sanitize_text_field( wp_unslash( $_SERVER['HTTP_REFERER'] ) ) : '',
					'Query String' => isset( $_SERVER['QUERY_STRING'] ) ? sanitize_text_field( wp_unslash( $_SERVER['QUERY_STRING'] ) ) : '',
				),
				'Authorizing Env'
			);
			$this->wp_bitly_logger->wpbitly_debug_log(
				array(
					'access_token'         => $token,
					'login'                => $login,
					'Escaped access_token' => esc_attr( $token ),
				),
				'Authorizing'
			);
		...
		if ( $this->wp_bitly_auth->is_authorized() && isset( $_GET['disconnect'] ) && 'bitly' === $_GET['disconnect'] ) {

			$nonce = isset( $_GET['nonce'] ) ? sanitize_text_field( wp_unslash( $_GET['nonce'] ) ) : '';
			if ( ! wp_verify_nonce( $nonce, 'bitly_disconnect' ) ) {
				return;
			}
	...
	public function regenerate_links() {
		...
		if ( isset( $_GET['wpbr'] ) && isset( $_GET['post'] ) ) {
			...
			if ( ! wp_verify_nonce( $nonce, 'wpbitly_regenerate' ) ) {
				return false;
			}

			if ( ! $this->wp_bitly_auth->is_authorized() || ! is_numeric( $_GET['post'] ) ) {
				return false;
			}
