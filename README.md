...
... will update the *ACF to Rest API* plugin to version 3.1.0.

Updating plugins individually is recommended when testing for compatibility. However, to update all of the packages, run `composer update`.

### WordPress Admin Plugins
Not all plugins can be managed by Composer. These plugins must be updated by either downloading from the [WordPress Plugins Page](https://wordpress.org/plugins/) and adding directly to the `wp-content/plugins` directory or logging into the `WordPress Admin` and updating via the `/wp-admin/plugins` page.
...
Additional configuration and functions can be found in the ACCESS NYC Theme and are described below.
...
#### PHP Functions
Some functions are included in the main [`functions.php`](https://github.com/CityOfNewYork/ACCESS-NYC/blob/master/wp-content/themes/access/functions.php), however, new modules are stored in the [`/includes`](https://github.com/CityOfNewYork/ACCESS-NYC/tree/master/wp-content/themes/access/includes) directory and are included in `functions.php` at the bottom of the file.

#### Twig Templates
The ACCESS NYC Theme is built on [Timber](https://www.upstatement.com/timber/) which uses the [Twig Templating Engine](https://twig.symfony.com/). Templates can be edited in the [`/views`](https://github.com/CityOfNewYork/ACCESS-NYC/tree/master/wp-content/themes/access/views) directory.
...
The NPM package comes with three NPM scripts. Each can be run via the command:
...
#### PHP
PHP is linted using [PHP Code Sniffer](https://github.com/squizlabs/PHP_CodeSniffer) with the [PSR-2 standard](https://www.php-fig.org/psr/psr-2/). The configuration can be found in the [phpcs.xml](https://github.com/CityOfNewYork/ACCESS-NYC/blob/master/phpcs.xml). The composer.json file includes two scripts for linting and fixing code. Each can be run via the command:
